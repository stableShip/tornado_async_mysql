...
Edited by haoming li:
    - Connection pool support with PooledDB implemented.
    - The first row of result will be returned when 'get()' function returns multi rows,
...
def pool(host, database, port=3306, user="root", password=None, mincached=5,):

    return PooledDB.PooledDB(host=host, db=database, port=port, user=user,
                                passwd=password, creator=MySQLdb, mincached=5)
...
class Connection(object):
    ...
    columns can be accessed by name. Typical usage::
        ...
        for article in db.query(u"selecct * FROM articles"):
            print article.title
    ...
    def __init__(self, host, database, user=None, password=None,
                 ...
                 time_zone="+8:00", charset = "utf8", sql_mode="TRADITIONAL", pool=None):
        self.host = host
    ...
    def reconnect(self):
        ...
        self.close()
        try:
            # pool = PooledDB.PooledDB(creator=MySQLdb, mincached=5,  **self._db_args)
            if self._pool:
                # print("in pool")
